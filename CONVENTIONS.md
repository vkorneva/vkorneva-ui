...
## Оглавление
  ...
  1. [HTML, SCSS, CSS](#scss-conventions)
...
## <a name="destructuring">Деструктуризация</a>
  ...
  - [5.1](#destructuring--object) При обращении к нескольким свойствам объекта предпочтительно использовать деструктуризацию объекта. eslint: [`prefer-destructuring`](https://eslint.org/docs/rules/prefer-destructuring)
...
## <a name="variables">Переменные</a>
  ...
  - [13.6](#variables--unary-increment-decrement) Избегайте использования унарных инкрементов и декрементов (`++`, `--`) (не распространяется на условия для for). eslint [`no-plusplus`](https://eslint.org/docs/rules/no-plusplus)
...
## <a name="blocks">Блоки</a>
  ...
  - [16.2](#blocks--cuddled-elses) Если блоки кода в условии `if` и `else` занимают несколько строк, расположите оператор `else` на следующей строчке, после закрывающей фигурной скобки блока `if`. eslint: [`brace-style`](https://eslint.org/docs/rules/brace-style.html)
...
## <a name="whitespace">Пробелы</a>
  ...
  - [19.1](#whitespace--spaces) Используйте мягкую табуляцию (символ пробела) шириной в 4 пробела. eslint: [`indent`](https://eslint.org/docs/rules/indent.html)
  ...
  - [19.9](#whitespace--in-parens) Добавляйте пробелы между круглыми скобками и их содержимым. eslint: [`space-in-parens`](https://eslint.org/docs/rules/space-in-parens.html)
  ...
  - [19.12](#whitespace--max-len) Старайтесь не допускать, чтобы строки были длиннее 120 символов (включая пробелы). Замечание: согласно [пункту выше](#strings--line-length), длинные строки с текстом освобождаются от этого правила и не должны разбиваться на несколько строк. eslint: [`max-len`](https://eslint.org/docs/rules/max-len.html)
...
## <a name="scss-conventions">HTML, CSS, SCSS</a>
  - **Правила именования классов**
    - Имена записываются латиницей в нижнем регистре.
    - Для разделения слов в именах используется дефис (-).
    - Имя блока задает пространство имен для его элементов и модификаторов.
    - Имя элемента отделяется от имени блока одним подчеркиваниями (_).
    - Имя модификатора отделяется от имени блока или элемента дефисом (-).
    ...
    - В конце имени класса должен стоять суффикс bvk: btn-bvk, btn-link-bvk

  - **ЗАПРЕЩЕНО**:
    - Использовать стили на "теги" и в "тегах" (inline-стили).
    - Использовать стили на id. (id - только для js). stylelint: [`selector-max-id`](https://stylelint.io/user-guide/rules/selector-max-id/)
    - Использовать !impotent. stylelint: [`declaration-no-important`](https://stylelint.io/user-guide/rules/declaration-no-important/)
    ```
      // плохо html
      ...
      #some-id { padding: 10px; }
      .some-class { padding: 10px !importent; }
    ```

  - Один селектор в строке.
  
    Селектор после запятой всегда на новой строке. stylelint: [`selector-list-comma-newline-after`](https://stylelint.io/user-guide/rules/selector-list-comma-newline-after/)
    ```
      .selector-one,
      .selector-two {
      }
    ```
  - Каждое свойство на новой строке. stylelint: [`declaration-block-semicolon-newline-after`](https://stylelint.io/user-guide/rules/declaration-block-semicolon-newline-after/)
    ```
       // плохо
        a,
  ...
  - Размещать блок в одной строке с последним селектором, если свойство одно. stylelint: [`declaration-block-single-line-max-declarations`](https://stylelint.io/user-guide/rules/declaration-block-single-line-max-declarations/#declaration-block-single-line-max-declarations)
    ```
        // плохо
        a,
        b { color: pink; top: 3px; }
  ...
  - Во избежании дублирования кода использовать миксины только если они должны принимать аргументы и только для уменьшения кода.

  - Переменные в scss только с дефисом.
  - @extend следует избегать, поскольку он имеет не интуитивное и потенциально опасное поведение, особенно при использовании с вложенными селекторами.

  - Не вкладывать селекторы глубже чем на три уровня!
    ...
    ```
  - Использовать двойные ковычки. stylelint: [`string-quotes`](https://stylelint.io/user-guide/rules/string-quotes/#string-quotes)
  - Не использовать нативные названия цветов. stylelint: [`color-named`](https://stylelint.io/user-guide/rules/color-named/#color-named)
  - Отступы: 2 пробела. stylelint: [`indentation`](https://stylelint.io/user-guide/rules/indentation/#indentation)

  - **Порядок в селекторе**
    1. Перечислите все стандартные объявления свойств, все, что не является @include или вложенным селектором.
    1. @include - Группирование @includes в конце облегчает чтение всего селектора.
        ```
        .btn-green {
          background: green;
          font-weight: bold;
    ...
    1. Вложенные селекторы, если необходимо, идут последними, и после них ничего не происходит. Добавьте пробелы между объявлениями правил и вложенными селекторами, а также между соседними вложенными селекторами. Примените те же рекомендации, что и выше, к вашим вложенным селекторам.
